...
1. Save the complete review to a file named `egenakademi-review-[date].md` in the project root
...
## EgenAkademi Project-Specific Review Rules

**CRITICAL:** These rules are mandatory for all code reviews in this project. Violations MUST be flagged as High or Critical severity.

### Branding System
- **No hardcoded colors:** All `.module.css` files MUST use `var(--color-*)` for theming colors. Flag any hex/rgba values that represent theme colors (exceptions: neutral backgrounds for previews like logo sjakkmoenster).
- **Sidebar context:** Elements inside the sidebar MUST use `--color-sidebar-*` variables, NOT `--color-text-*`.
- **Alpha blending:** Use `color-mix(in srgb, var(--color-*) N%, transparent)` instead of rgba with hardcoded colors.
- **New color fields** require updates in 6 locations: Prisma schema, `CSS_VAR_MAP` in layout.tsx, globals.css defaults, `BrandingData` interface, `BRANDING_FIELDS`, `SECTIONS` array, and generator.

### Multi-Tenancy & Security
- **Always check `session.user.tenantId`** in every server action and API route. Missing tenant check = Critical severity.
- **Server actions** MUST return `{ success: true }` or `{ error: string }` — never throw unhandled errors to client.
- **Never expose stack traces** — catch all errors, return generic messages, log server-side.
- **CORS must never be `*`** — use allowlist from env variables.
- **Redirect validation** — never redirect to arbitrary URLs from query parameters.

### Auth Architecture
- **Edge-safe splitting:** `middleware.ts` imports ONLY from `auth.config.ts` — never from `auth.ts` or Prisma. Violation = Critical.
- **JWT contents:** `id`, `tenantId`, `globalRole`, `tenantPlan` — verify these are available before use.

### Database & ORM
- **Prisma v7 pattern:** Must use `@prisma/adapter-pg` + `pg` pool. Check that `src/lib/prisma.ts` singleton is used.
- **Tenant scoping:** All database queries MUST include `tenantId` in WHERE clause. Missing = Critical.

### UI Standards
- **Icons:** Only `lucide-react` — flag any emoji usage in UI components.
- **Server Components default:** Flag unnecessary `'use client'` directives — only add for interactivity.
- **WCAG AA:** Verify 4.5:1 contrast for text, 3.0:1 for interactive elements, visible focus states.
- **CSS Modules:** All component styling via `.module.css` files using CSS variables.

### Code Patterns
- **Feature gating:** Use `src/lib/features.ts` helpers for plan-based feature checks.
- **Norwegian language:** All UI text and user-facing strings should be in Norwegian.
